"""
YOLOE PPE (Helmet & Vest) 파인튜닝 스크립트

사용법:
    python train_ppe_yoloe.py                    # 기본 설정으로 학습
    python train_ppe_yoloe.py --model yolo11s.pt # 다른 모델 사용
    python train_ppe_yoloe.py --epochs 200       # 에폭 변경
    python train_ppe_yoloe.py --resume           # 이어서 학습
...
def parse_args():
    parser = argparse.ArgumentParser(description="PPE Helmet & Vest YOLO Training")

    # 모델 설정
    parser.add_argument("--model", type=str, default="yoloe-26m-seg.pt",
                        help="모델 파일 (yoloe-26n/s/m/l/x-seg.pt)")
    parser.add_argument("--data", type=str, default="dataset/ppe_helmet_vest_unified/data.yaml",
                        help="데이터셋 yaml 경로")
    ...
    # 프로젝트 설정
    parser.add_argument("--project", type=str, default="runs/ppe_helmet_vest", help="프로젝트 폴더")
...
def train(args):
    """모델 학습"""
    print("="*60)
    print("PPE Helmet & Vest Detection Training")
    print("="*60)
...
def validate(args):
    ...
    print("\n클래스별 AP50:")
    class_names = ['Helmet_OFF', 'Helmet_ON', 'Vest_OFF', 'Vest_ON']
